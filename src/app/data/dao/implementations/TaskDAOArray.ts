...
// находит последний id (чтобы потом вставить новую запись с id, увеличенным на 1) - в реальной БД это происходит автоматически
  ...
  search(category: Category, searchText: string, status: boolean, priority: Priority): Observable<Task[]> {
    return of(this.fillTasksByParams(category, searchText, status, priority));
  }

  fillTasksByParams(category: Category, searchText: string, status: boolean, priority: Priority): Task[] {
